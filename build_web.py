...
<div class="wrap">
  ...
  <div class="topbar">
    ...
    <a class="btn" href="community.html" title="Community chat &amp; private inbox">👥 Community</a>
...
function userTrophyCount(u){
  if(SERVER && u && !u.hasPrivateProgress) return Number(u.trophyCount)||0;
  return trophyCount((u&&u.progress)||{done:{}});
}
function primeAll(){
  rebuildTasks();
...
function renderChallenges(){
  ...
  host.innerHTML='<h2 class="sec">Weekly Challenges</h2>'+cs.map(c=>{
    ...
    return `<div class="${cls}">
      <div class="chIco">${escapeHtml(troph?troph.ico:"🎯")}</div>
      <div class="chBody">
        ...
        ${c.goalType==="tasks"&&(c.taskIds||[]).length?`<div class="chTasks">${c.taskIds.map(id=>`<span class="chip id${state.done[id]?" met":""}">${escapeHtml(id)}</span>`).join("")}</div>`:""}
      </div></div>`;
  }).join("");
}
...
function renderMyTasks(){
  ...
  const rows=list.map(t=>{ const d=!!state.done[t.id];
    return `<div class="task${d?" done":""}">
      <div class="chk" data-id="${escapeAttr(t.id)}" role="checkbox" aria-checked="${d}" tabindex="0">${checkSvg()}</div>
      ...
      <button class="btn mini danger delMy" data-id="${escapeAttr(t.id)}" title="delete task">✕</button></div>`;
  }).join("");
    ...
    for(const t of shown){
      ...
      row.innerHTML=`
        ...
        <div class="tbody">
          ...
          <div class="tmeta">
            <span class="chip id">${escapeHtml(t.id)}</span>
            ...
            ${hasGuide?`<button class="gBtn" data-guide="${escapeAttr(t.id)}" aria-expanded="false">📖 Guide</button>`:""}
          </div>
        </div>
        <div class="xpTag">${t.xp} XP</div>
        <div class="taskGuide" data-gid="${escapeAttr(t.id)}" hidden></div>`;
...
function renderTrophies(){
  ...
  for(const t of allTrophies()){
    ...
    el.innerHTML=`<div class="ribbon">${won?"★":"🔒"}</div><div class="tico">${escapeHtml(t.ico)}</div>
...
function toast(kind,tt,ts){
  ...
  const title=document.createElement("div"); title.className="tt"; title.textContent=String(tt||"");
  const sub=document.createElement("div"); sub.className="ts"; sub.textContent=String(ts||"");
...
function openUserMenu(){
  const rows=Object.values(DB.users).map(u=>{ const c=userStats(u);
    return `<div class="lbRow ${u.id===DB.currentUser?"me":""}" data-act="switch" data-id="${escapeAttr(u.id)}" style="cursor:pointer">
      <span class="lbAv">${escapeHtml(u.avatar||"🎯")}</span>
...
function shareData(){
  ...
  return { name:(u.name||"Operator"), avatar:(u.avatar||"🎯"), level:L, xp:xp, pool:pool,
    ...
    pct:(total?Math.round(done/total*100):0), trophies:userTrophyCount(u) };
}
...
function openLeaderboard(){
  const arr=Object.values(DB.users).map(u=>({u,c:userStats(u),tr:userTrophyCount(u)}))
    .sort((a,b)=>b.c.xp-a.c.xp);
  const medal=i=> i===0?"🥇":i===1?"🥈":i===2?"🥉":("#"+(i+1));
  const rows=arr.map((r,i)=>`<div class="lbRow ${r.u.id===DB.currentUser?"me":""}">
    <span class="lbPos">${medal(i)}</span><span class="lbAv">${escapeHtml(r.u.avatar||"🎯")}</span>
...
function adminUsers(){
  const rows=Object.values(DB.users).map(u=>{ const c=userStats(u);
    ...
    const acts = SERVER
      ? `<button class="btn mini" data-act="urename" data-id="${escapeAttr(u.id)}">Rename</button>
         <button class="btn mini" data-act="upw" data-id="${escapeAttr(u.id)}">Set password</button>
         <button class="btn mini" data-act="uadmin" data-id="${escapeAttr(u.id)}">${u.isAdmin?"Revoke admin":"Make admin"}</button>
         <button class="btn mini" data-act="ureset" data-id="${escapeAttr(u.id)}">Reset</button>
         <button class="btn mini danger" data-act="udel" data-id="${escapeAttr(u.id)}">Delete</button>`
      : `<button class="btn mini" data-act="uswitch" data-id="${escapeAttr(u.id)}">Switch</button>
         <button class="btn mini" data-act="urename" data-id="${escapeAttr(u.id)}">Rename</button>
         <button class="btn mini" data-act="uadmin" data-id="${escapeAttr(u.id)}">${u.isAdmin?"Revoke admin":"Make admin"}</button>
         <button class="btn mini" data-act="ureset" data-id="${escapeAttr(u.id)}">Reset</button>
         <button class="btn mini danger" data-act="udel" data-id="${escapeAttr(u.id)}">Delete</button>`;
    return `<tr>
      <td>${escapeHtml(u.avatar||"🎯")} <b>${escapeHtml(u.name)}</b>${isMe?' <span class="badgePill">you</span>':""}</td>
      <td>${u.isAdmin?"✓":""}</td>
      <td class="mono">${c.level}</td><td class="mono">${c.xp.toLocaleString()}</td>
      <td class="mono">${c.rmDone}/${c.rmTotal}</td><td class="mono">${userTrophyCount(u)}</td>
      <td>${acts}</td></tr>`;
  }).join("");
...
function adminTasks(){
  const rows=TASKS.map(t=>{ const isC=DB.customTasks.some(x=>x.id===t.id); const ov=!!DB.overrides[t.id];
    return `<tr>
      ...
      <td><button class="btn mini" data-act="tedit" data-id="${escapeAttr(t.id)}">Edit</button>
      <button class="btn mini danger" data-act="tdel" data-id="${escapeAttr(t.id)}">Delete</button></td></tr>`;
  }).join("");
  return taskForm()+
...
function ruleText(r){ r=r||{type:"manual"};
  switch(r.type){ case "xp":return "XP ≥ "+r.value; case "level":return "Lvl ≥ "+r.value;
    ...
    case "ids":return escapeHtml((r.value||[]).join(",")); case "all":return "100% roadmap"; default:return "manual grant"; } }
function adminTrophies(){
  const list=allTrophies().map(t=>{ const isC=(DB.customTrophies||[]).some(x=>x.id===t.id);
    ...
    return `<tr>
      <td>${escapeHtml(t.ico)} <b>${escapeHtml(t.name)}</b>${isC?' <span class="badgePill">custom</span>':""}</td>
      <td>${escapeHtml(t.desc||"")}</td>
      <td style="color:${c}">${escapeHtml((TIERS[t.tier]||{}).n||t.tier||"")}</td>
      <td class="mono">${ruleText(t.rule)}</td>
      <td>${trophyEarned(t)?"✓":""}</td>
      <td>${isC?`<button class="btn mini danger" data-act="trophdel" data-id="${escapeAttr(t.id)}">Delete</button>`:""}</td></tr>`;
  }).join("");
  const uopts=Object.values(DB.users).map(u=>`<option value="${escapeAttr(u.id)}">${escapeHtml(u.name)}</option>`).join("");
  const topts=allTrophies().map(t=>`<option value="${escapeAttr(t.id)}">${escapeHtml(t.name)}</option>`).join("");
  return `<div class="fbox">
    <div class="row">
...
function adminChallenges(){
  const rows=(DB.challenges||[]).map(c=>{ const w=chalWindow(c); const troph=allTrophies().find(t=>t.id===c.trophyId);
    return `<tr>
      ...
      <td>${troph?escapeHtml(troph.ico)+" "+escapeHtml(troph.name):"—"}</td>
      <td>${c.active?(w.expired?'<span style="color:var(--red)">expired</span>':'<span style="color:var(--greenb)">active</span>'):"off"}</td>
      <td>
        <button class="btn mini" data-act="chtoggle" data-id="${escapeAttr(c.id)}">${c.active?"Disable":"Enable"}</button>
        <button class="btn mini" data-act="chrestart" data-id="${escapeAttr(c.id)}">Restart</button>
        <button class="btn mini danger" data-act="chdel" data-id="${escapeAttr(c.id)}">Delete</button></td></tr>`;
  }).join("");
  return `<div class="fbox">
    <div class="row">
...
function adminData(){
  if(SERVER){
    return `<div class="fbox"><b style="font-size:12px;color:#fff">Change your password</b><div class="row" style="margin-top:6px">
      ...
      <div class="note">Export writes every account (incl. password hashes) + shared config to one JSON file — sensitive, keep it safe. Imported legacy accounts start disabled and non-admin; set each password explicitly before use. Wipe deletes all <b>other</b> accounts, clears shared config, and resets your own progress.</div>`;
  }
...
function importLegacyHandler(e){          // server mode: seed accounts from browser "Export ALL"
  ...
  r.onload=async ()=>{ let d; try{ d=JSON.parse(r.result); }catch(err){ alert("Invalid file"); return; }
    ...
    toast("task","Imported "+(res.j.created||0)+" account(s)","set each password before use"); };
  r.readAsText(file);
}
...
async function onAdminClick(e){
  ...
  switch(act){
    ...
    case "chpw": { if(!SERVER) return; const r=await api("POST","/api/me/password",{old:g("pwOld").value,new:g("pwNew").value});
        if(!r.ok) return fail(r); location.reload(); return; }
    case "exportAll":
...
function applyServerState(s){
  ...
  for(const [idk,u] of Object.entries(s.users||{})){
    DB.users[idk]={ id:idk, name:u.name, avatar:u.avatar, isAdmin:u.isAdmin,
                    stats:u.stats||null, trophyCount:Number(u.trophyCount)||0,
                    hasPrivateProgress:Object.prototype.hasOwnProperty.call(u,"progress"),
                    progress:normProg(u.progress||{}) };   // private progress only for self/admin
  }
  reselect();
}
...
function openAccountMenu(){
  ...
  ov.addEventListener("click",async e=>{
    ...
    if(act==="chpw"){ const r=await api("POST","/api/me/password",{old:$("#pwOld").value,new:$("#pwNew").value});
      if(!r.ok) toast("task","Failed",(r.j&&r.j.error)||""); else location.reload(); }
  });
}
