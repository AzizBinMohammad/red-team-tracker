...
function adminRender(){
  ...
  const inner=`<div class="mhead"><h3>⚙ Admin Panel</h3><span class="badgePill">local · soft-gated</span>
      <span class="x" data-act="close">✕</span></div>
    <div class="note">This panel and its PIN live only in this browser — a convenience gate for a shared machine, <b>not real access control</b>. Anyone with devtools can bypass it.</div>
  ...
  if(adminTab==="data"){ const f=document.getElementById("importAllFile"); if(f) f.onchange=importAllHandler; }
}

function adminUsers(){
  const rows=Object.values(DB.users).map(u=>{ const c=ctxOf(u.progress);
    return `<tr>
      <td>${u.avatar||"🎯"} <b>${escapeHtml(u.name)}</b>${u.id===DB.currentUser?' <span class="badgePill">current</span>':""}</td>
      <td>${u.isAdmin?"✓":""}</td>
  ...
  return `<table class="tbl"><tr><th>Profile</th><th>Admin</th><th>Lvl</th><th>XP</th><th>Roadmap</th><th>🏆</th><th>Actions</th></tr>${rows}</table>
...
function saveTask(){
  ...
  save(); rebuildTasks(); renderAll(); adminRender();
}
function addTrophy(){
  const g=id=>document.getElementById(id);
...
function addChallenge(){
  ...
  save(); rebuildTasks(); renderAll(); checkChallenges(); adminRender();
}
function exportAll(){
...
function onAdminClick(e){
  ...
  const rerender=()=>{ save(); rebuildTasks(); reselect(); renderAll(); adminRender(); };
  switch(act){
    case "close": return closeModal();
    case "tab": adminTab=a.dataset.tab; editTask=null; return adminRender();

    // profiles
    case "uswitch": DB.currentUser=id; reselect(); save(); renderAll(); checkChallenges(); return adminRender();
    case "urename": { const n=(prompt("Rename profile:",DB.users[id].name)||"").trim(); if(n){ DB.users[id].name=n; rerender(); } return; }
    case "uadmin": DB.users[id].isAdmin=!DB.users[id].isAdmin; return rerender();
    case "ureset": if(confirm("Reset ALL progress for "+DB.users[id].name+"?")){
    ...
    case "uadd": { const n=(g("newUserName").value||"").trim(); if(!n){ alert("Name required"); return; }
    ...
    case "tdel": {
        ...
        delete DB.overrides[id]; if(editTask===id) editTask=null; return rerender(); }
    case "trestore": if(confirm("Restore built-in tasks and discard all edits/deletions? Custom tasks are kept.")){
        DB.deleted=DB.deleted.filter(x=>DB.customTasks.some(c=>c.id===x)); DB.overrides={}; editTask=null; rerender(); } return;

    // trophies
    case "trophadd": return addTrophy();
    case "trophdel": DB.customTrophies=(DB.customTrophies||[]).filter(x=>x.id!==id); return rerender();
    case "grgrant": { const uu=g("grUser").value, tt=g("grTroph").value;
        ...
        toast("task","Trophy granted",DB.users[uu].name); return rerender(); }
    case "grrevoke": { const uu=g("grUser").value, tt=g("grTroph").value;
        delete DB.users[uu].progress.granted[tt]; return rerender(); }
    ...
    case "chdel": { const c=(DB.challenges||[]).find(x=>x.id===id);
        ...
        DB.challenges=(DB.challenges||[]).filter(x=>x.id!==id); return rerender(); }
    case "chtoggle": { const c=(DB.challenges||[]).find(x=>x.id===id); if(c) c.active=!c.active; return rerender(); }
    case "chrestart": { const c=(DB.challenges||[]).find(x=>x.id===id);
        if(c){ c.startedAt=Date.now(); for(const u of Object.values(DB.users)){ if(u.progress.challengesDone) delete u.progress.challengesDone[id]; } }
        return rerender(); }

    // data
    case "setpin": { const p=(g("newPin").value||"").trim(); if(p){ DB.settings.adminPin=p; save(); toast("task","PIN updated",""); } return; }
    case "exportAll": return exportAll();
    case "importAll": return g("importAllFile").click();
    case "wipe": if(confirm("WIPE everything — all profiles, edits, trophies, challenges? This cannot be undone.")){
...
// ================= boot =================
function init(){ rebuildTasks(); primeAll(); initTopbar(); initToolbar(); renderAll(); checkChallenges(); }
init();
</script>
</body>
</html>
